...
#[get("/<filename>")]
fn res(filename: String) -> Option<NamedFile> {
    let exepath = env::current_dir().unwrap();
...
fn sendmail(user_input: Data) -> Html<&'static str> {
    ...
    for line in lines {
        if getsender {
            sender = line.unwrap().clone();
            getsender = false;
            continue;
        }
        if getsenderemail {
            senderemail = line.unwrap().clone();
            getsenderemail = false;
            continue;
        }
        if getpwd {
            pwd = line.unwrap().clone();
            getpwd = false;
            break;
        }
    }
    ...
    match mailer.send(&emmail) {
        ...
        Err(e) => {
            ...
            println!("[ERROR 001]========================");
            return Html(
                r#"<html>Message not sended due an some error 001. Please contact support.
        <script>
        setTimeout(function () {
    ...
    match mailer.send(&emmail) {
        Ok(_) => Html(
            r#"<html>Message succefully sedended.
        <script>
        setTimeout(function () {
            window.location.href = "/"; //will redirect to your blog page (an ex: blog.html)
         }, 3000);
        ...
        Err(e) => {
            ...
            println!("[ERROR 002]========================");
            return Html(
                r#"<html>Message not sended due an some error 002. Please contact support.
...
fn main() {
    ...
    rocket::custom(cfg)
        .mount(
            "/",
            routes![index, exit, sendmail, img, css, js, video, resource, res],
        )
        .launch();
}
